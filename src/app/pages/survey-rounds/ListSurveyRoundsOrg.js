...
const ListSurveyRoundsORG = (props) => {
  ...
  const searchanization = (dataSearch = {}) => {
    setLoading(true);
    makeRequest("get", `surveyroundsorg/getAllSurRoundsOrg`, dataSearch)
      .then(({ data }) => {
        if (data.signal) {
  ...
  const deleteAction = (id) => {
    ...
    hideDeleteModal();
    setLoadDelete(false);
    makeRequest("get", `surveyroundsorg/deleteSurRoundsOrg`, { id: dataDelete.id })
      .then(({ data }) => {
        console.log(data);
        if (data.signal) {
  ...
  const detailSurveyRoundsOrg = (row) => {
    setShowDetail(true);
    setDataDetail(row);
  };
              ...
              <Paper className={classes1.root}>
                ...
                {isLoading ? (
                  ...
                  <Table className={classes1.table}>
                    <TableHead>
                      <TableRow>
                        <TableCell>Số id Rounds</TableCell>
                        <TableCell>Số id Organization</TableCell>
                      ...
                      {rows && rows.length ? (
                        rows.map((row, key) => (
                          <TableRow key={`surveyroundsorg-${key}`}>
                            <TableCell>{row.survey_round_id}</TableCell>
            ...
            >
              ...
              >
                <Card>
                  <div className="row">
                    <div className="col-md-12">
                      <span>Mã đợt khảo sát: {dataDetail.organization_id}</span>
                    </div>
                    <div className="col-md-12">
                      <span>
                        Tên đợt khảo sát: {dataDetail.survey_round_id}
                      </span>
                    </div>
                 
                    <div className="col-md-12">
                      <span>Khảo sát tạo ngày: {dataDetail.createdAt}</span>
                    </div>
                    <div className="col-md-12">
                      <span>
                       Khảo sát cập nhật ngày: {dataDetail.updatedAt}
                      </span>
                    </div>
                    <div className="col-md-12">
                      <span>
...
export default connect(mapStateToProps, null)(ListSurveyRoundsORG);
