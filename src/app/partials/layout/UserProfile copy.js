...
class UserProfile extends React.Component {
  ...
  componentDidMount() {
    const token = JSON.parse(JSON.parse(localStorage.getItem('persist:vietlook-auth')).authToken);
    makeRequest('get', `api/user/getUserCurrent?token=${token}`)
      .then(({ data }) => {
        let detail = data.data[0];
        this.setState({ username: detail.username, photo: detail.photo });
      })
      .catch(err => {
        console.log(err);
      });
      ...
      >
        <Dropdown.Toggle as={HeaderDropdownToggle} id="dropdown-toggle-user-profile">
          <div className="kt-header__topbar-user">
            ...
            {showAvatar && <img alt="Pic" src={this.state.photo} />}
